...
const staticPaths = [
  path.join(__dirname, 'public'),
  path.join(__dirname, '..', 'frontend'), // <- serve the dashboard assets too
];
...
app.get(['/dashboard', '/dashboard.html'], (req, res) => {
  // Prefer frontend/dashboard.html; fall back to backend/public if needed
  const candidates = [
    path.join(__dirname, '..', 'frontend', 'dashboard.html'),
    path.join(__dirname, 'public', 'dashboard.html'),
  ];
  const hit = candidates.find(p => fs.existsSync(p));
  if (!hit) return res.status(404).type('text/plain').send('dashboard.html not found');

  // Optional: also inject TURN (helps the desktop page if you load it here)
  const html = fs.readFileSync(hit, 'utf8');
  res.send(injectTurnConfig(html));
});
// -------------------- TURN Injection --------------------
function injectTurnConfig(html) {
...
app.get('*', (req, res) => {
  ...
  // Only block dashboard paths in development (so port :8080 isn't used for UI locally)
  if (!IS_PROD && (p === '/dashboard' || p === '/dashboard.html' || p.startsWith('/dashboard/'))) {
    return res.status(404).type('text/plain').send('Not Found');
  }
